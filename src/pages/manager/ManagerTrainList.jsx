...
const ManagerTrainList = () => {
  ...
  useEffect(() => {
    const fetchTrainList = async () => {
      try {
        const response = await axios.get(`http://localhost:8091/api/manager/KDT/${kdtSessionId}/train/list`,{ // API 엔드포인트 수정
          headers: {
 ...
 const filteredTrainList = trainList.filter((item) => {
  ...
  return (
      ...
      <tbody>
        {currentItems.map((item, index) => (
              <tr key={item.kdtTrainId}>
                ...
                <td><a href={`http://localhost:8091/manager/KDT/${item.kdtSessionId}/train/${item.kdtTrainId}`}>{item.kdtTrainTitle}</a></td>
                <td>{item.kdtTrainSubject}</td>
                <td>{item.kdtTrainContent}</td>
                <td>{formatDate(item.kdtTrainDate)}</td>
                <td><a href={`http://localhost:8091/manager/KDT/${item.kdtSessionId}/train/${item.kdtTrainId}`}>상세보기</a></td>
              </tr>
            ))}
      </tbody>
    </table>
    ...
    <div className={styles.buttonContainer}>
      <a
        href={`http://localhost:8091/manager/KDT/${kdtSessionId}/train`}
