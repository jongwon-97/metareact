...
const TestList = () => {
  ...
  useEffect(() => {
    const  fetchTestList = async () => {
      try {
        const response = await axios.get(`http://localhost:8091/api/admin/KDT/${kdtSessionId}/test/list`,{ // API 엔드포인트 수정
          headers: {
        ...
        <tbody>
          {currentItems.map((test, index) => (
            <tr key={test.kdtTestId}>
              ...
              <td>
                <a href={`http://localhost:8091/admin/KDT/${kdtSessionId}/test/${test.kdtTestId}`}>
                  {test.kdtTestTitle}
                </a>
              </td>
